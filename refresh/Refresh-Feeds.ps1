...
$status = [ordered]@{
  ...
  schedule  = if ($env:GITHUB_ACTIONS -eq 'true') { if ($env:GITHUB_EVENT_NAME -eq 'schedule') { 'Daily (GitHub Actions)' } else { 'On demand (GitHub Actions)' } } elseif ($env:AMI_SCHEDULED -eq '1') { 'Daily (Windows Task Scheduler)' } else { 'Manual run' }
  feeds = $feedResults
